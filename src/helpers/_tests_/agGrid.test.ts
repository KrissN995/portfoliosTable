import {getGridTheme, valueCellRenderer, aggregateValues} from '../agGrid';
import {Client} from "../../models/appModels";
import {aggregateRestrictionStatus} from "../agGrid";
...
test('getGridTheme_should_return_ag-theme-alpine-dark', () => {
    const result = getGridTheme(true);
    expect(result).toBe('ag-theme-alpine-dark');
});

test('valueCellRenderer_should_return_1,200.24 EUR', () => {
    const result = valueCellRenderer(mockData);
    expect(result).toBe('1,200.24 EUR');
});

test('aggregateValuesAgGrid_should_return_17656587.4', () => {
    ...
    const result = aggregateValues(mockData, 'capitalGain', currencyExchangeRated);
    expect(result.toFixed(1)).toBe("17656587.4");
});

test('aggregateRestrictionStatusAgGrid_should_return_Clean, Breached', () => {
    const result = aggregateRestrictionStatus(mockData);
    console.log(result);
    expect(result.length).toBe(2);
    expect(result[0]).toBe('Clean');
    expect(result[1]).toBe('Breached');
});